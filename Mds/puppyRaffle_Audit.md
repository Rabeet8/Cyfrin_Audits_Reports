...
A player who has entered the raffle could have a `fallback/recieve` function calls the `PuppyRaffle::enterRaffle` function again and claim another refund.They could continue the cycle till the contract balance is drained.
...
To prevent this, we should have the `PuppyRaffle:refund` function update the `players` array before making the external call. Additionally we should move teh event emission up as well.
...
2. User can mine/manupilate their `msg.sender` value to result in their address being used to generated the winner.

3. Users can revert their `selectWinner` transaction if they don't like the winner or resulting puppy.

Using on-chain values as a randomness seed is a well-documented vector.

**Proof Of Concept:**

Consider using a cryptographically hashing method like using the chainlink's VRFs
...
**Impact:**
In `PuppyRaffle::selectWinner`,`totalFees` are accumulated for the `feeAddress` to collect later in `PuppyRaffle::withdrawFees`. However, if the `totalFees` variable overflow, the `feeAddress` may not collect amount of fees,leaving fees permanently stuck in the contract.
...
4. You will not able to withdraw, due to line in `PuppyRaffle::withdrawFees`
...
There are more attack vectors with that final required, so we recommend removing it regardless
...
### [M-1] Potential DoS attack in `PuppyRaffle::enterRaffle` because of the unbound for-loop.

**Description:**

The `PuppyRaffle::enterRaffle` function loops through the `players` to check for duplicates.However the longer the `PuppyRaffle::players` array is, the more check a new player will have teh raffle state will be dramatically lower than those who enter later. Every additional address in the `players` array, is an additiona; check the loop will have to make.
...
The gas cost for raffle  entrant will greatly incease as more player enter the raffle.Discouraging later users from entering, and causing a rush at the start of a raffle to be one of the first entrants in the queue.
...
``` solidity
     ...
     function dosAttackOnEnterRaffle() public {
        vm.txGasPrice(1);
        ...
        console.log("Gas Used for first 100 person:", gasUsedFirst);
        // console.log("Gas Used for second 100 person:" gasUsedFirst);
    }
```
</details>
...
1. Consider allowing duplicates. Users can make new wallet addresses anyways, so a duplicate check does not prevent the same person from entering multiple time, only the same wallet address

2. Consider using a mapping to check for duplicates. This would allow constant time lookup of whether a user has already entered

### [M-2]  Smart contract wallet raffles winner without a `receive` or `fallback` function will block the start of a new contest.
...
User could easily call the `selectWinner` function again and non-wallet entrants could enter, but it could cost a lot due to the duplicate check and a lottery ret could get very challenging.

**Impact:**

The `PuppyRaffle::selectWinner` function could revery many time making a lottery rest difficult.

Also, true winners could not get paid out and someone else coul take out their money.
...
3. The `selectWinner` function would not even though the lottery is over.

**Recommended Of Mitigation:**

Few option to mitigate this issue.
